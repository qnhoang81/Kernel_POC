...
#define C__ "\x1b[0m \n"
static int modem_in_critical_state;

#define MODEM_SILENT_RESET

#endif

#define DPRAM_RECOVER_FROM_INT_ERR
...
static inline void check_int_pin_level(int cnt);

#ifdef DPRAM_RECOVER_FROM_INT_ERR
static int irqcnt;
#endif
...
//	down_interruptible(&write_mutex);	
	...
	if(writecnt++ > 3) {
		if(irqcnt == 0) {
			printk(KERN_ERR "[ONEDRAM] no irqs for multiple writes! Recovering...\n");
			check_int_pin_level(10);
		}
		irqcnt = 0;
		writecnt = 0;
	}
...
//				ret = len;
				ret = 0; //James/when tty channel is not open, dpram should drop data.
			}
...
#ifdef CONFIG_KERNEL_DEBUG_SEC	
void dpram_enter_upload(char* str_caller, u32 cause)
{
	static u32 modem_upload_processing = 0;
	char* msg;
	int msgidx;
	
	if(modem_upload_processing == 0) {
		
		modem_upload_processing = 1;

		if(cause == UPLOAD_CAUSE_CP_LOCKUP)
			msg = "lockup";
		else if (cause == UPLOAD_CAUSE_CP_WDOG)
			msg = "wdog";
		else if (cause == UPLOAD_CAUSE_CP_ERROR_FATAL)
			msg = "error fatal";

		printk(KERN_ERR __C "[%s] Modem %s" C__, str_caller, msg);
#ifdef MODEM_SILENT_RESET
		phone_sync = 0;
		requested_semaphore = 0;
		unreceived_semaphore = 0;
		dpram_phone_reset();
		modem_in_critical_state = false;
		modem_upload_processing = 0;
#else
		*onedram_mailboxBA = UPLOAD_AP_CP_NOTIFY_CODE;
		...
		kernel_sec_set_upload_cause(cause);

		printk(MSGDBG "[%s:%d] VMSMP2.6 = %d" MSGEND, __func__,__LINE__,
			gpio_get_value(GPIO_VMSMP_26V));
...
{
#ifdef CONFIG_KERNEL_DEBUG_SEC	
	if(phone_pwr_on == true) {
		dpram_enter_upload(__func__, UPLOAD_CAUSE_CP_LOCKUP);
		return;
	}
	else {
		printk(KERN_ERR __C "Modem is off. Ignore %s" C__, __func__);
		return;
	}
#endif
...
static int onedram_get_semaphore(const char *func)
{
...
#ifdef CONFIG_KERNEL_DEBUG_SEC	
	if(modem_in_critical_state) return -1;
#endif
	...
	printk("[OneDRAM](%s) Failed to get a Semaphore. sem:%d, PHONE_ACTIVE:%s, VMSMP:%s fail_cnt:%d\n", 
			func, *onedram_sem,	gpio_get_value(GPIO_PHONE_ACTIVE)?"HIGH":"LOW ", 
			gpio_get_value(GPIO_VMSMP_26V)?"HIGH":"LOW ", unreceived_semaphore);
...
static void dpram_phone_power_on(void)
{
	...
	/* We verified RST pin does not go to low when modem reset.
	    So, we should make PHONE_ON and RST pins low */
	gpio_set_value(GPIO_PHONE_ON, GPIO_LEVEL_LOW);
	gpio_set_value(GPIO_PHONE_RST_N, GPIO_LEVEL_LOW);
	mdelay(100);
...
static void dpram_phone_reset(void)
{
	...
	if(phone_pwr_on == false) {
		printk(KERN_ERR __C "Modem is off. Ignore %s" C__, __func__);
		return;
	}

	if(gpio_get_value(GPIO_VMSMP_26V) == 0) {
...
{
#ifdef CONFIG_KERNEL_DEBUG_SEC	
	modem_in_critical_state = true;
	printk(KERN_ERR __C "Modem in err_fatal(). Wait for upload mode" C__);
	return;
#endif
...
static inline
void check_int_pin_level(int cnt)
{
	u16 mask = 0, i = 0;
	...
	if(i > cnt-1) {
		printk(KERN_ERR "[OneDRAM] MailAB=%x\n", mask);
...
#ifdef CONFIG_KERNEL_DEBUG_SEC
	if(mbAB == (u32)('P' | ('O'<<8) | ('O'<<16) | ('F'<<24))) {
		phone_pwr_on = 0;
		printk(KERN_ERR __C "Modem power OFF" C__);
	}
	else if(mbAB == (u32)('P' | ('R'<<8) | ('S'<<16) | ('T'<<24))) {
		phone_pwr_on = 0;
		printk(KERN_ERR __C "Modem power RESET" C__);
	}
	else if(mbAB == UPLOAD_AP_CP_NOTIFY_CODE) {
		printk(KERN_ERR __C "ERR FATAL" C__);
		if(phone_pwr_on) {
			dpram_enter_upload(__func__, UPLOAD_CAUSE_CP_ERROR_FATAL);
			return IRQ_HANDLED;
		}
	}
#endif
...
static void phone_active_delayed_work_handler(struct work_struct *ignored)
{
	...
	if(gpio_get_value(GPIO_PHONE_ACTIVE) != 0) {
		printk(KERN_ERR __C "[%s] Phone active is now high! Ignored..." C__, __func__);
		return;
	}

	printk(KERN_ERR __C "Phone active is still low!!!" C__);
	
#ifdef CONFIG_KERNEL_DEBUG_SEC
	if(phone_sync && phone_pwr_on == true)
		dpram_enter_upload(__func__, UPLOAD_CAUSE_CP_WDOG);
	/* will not reach here */
#endif
...
static irqreturn_t phone_active_irq_handler(int irq, void *dev_id)
{
	...
	// modem bootup). So let's just call the delay function here for an easy fix
...
#ifdef CONFIG_KERNEL_DEBUG_SEC
	if(gpio_get_value(GPIO_PHONE_ACTIVE) == 0) {
		printk(KERN_ERR __C "Phone active is low" C__);
		if(phone_sync && phone_pwr_on == true) {
			dpram_enter_upload(__func__, UPLOAD_CAUSE_CP_WDOG);
			return IRQ_HANDLED;
		}
...
static void init_hw_setting(void)
{
	...
	// monitor VMSMP_26V
	if (gpio_is_valid(GPIO_VMSMP_26V)) {
		if (gpio_request(GPIO_VMSMP_26V, S3C_GPIO_LAVEL(GPIO_VMSMP_26V)))
			printk(KERN_ERR "Filed to request GPIO_VMSMP_26V!\n");
		gpio_direction_input(GPIO_VMSMP_26V);
	}
	s3c_gpio_setpull(GPIO_VMSMP_26V, S3C_GPIO_PULL_NONE); 
...
static void check_miss_interrupt(void)
{
	...
	if(extra_eint0pend == 0x00000080) {
		local_irq_save(flags);
		phone_active_irq_handler(IRQ_PHONE_ACTIVE, NULL);
		local_irq_restore(flags);
	}
