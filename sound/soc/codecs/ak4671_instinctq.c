...
#if 0// James defined(CONFIG_MACH_VINSQ)
	    if(system_rev>=CONFIG_INSTINCTQ_REV14) {
			gpio_direction_output(GPIO_MICBIAS_EN, 1);//kimhyuns hw bug	    
	    }
		else {
			gpio_direction_output(GPIO_MICBIAS_EN, 0);
		}
#elif 0 // defined(CONFIG_MACH_VITAL)
...
{
	P("AUDIO POWER : %d", en);
#if defined(CONFIG_MACH_MAX) //remove EAR_SEND_END IRQ
	if (en)
	{ 
...
int mic_enable(int en)
{
    ...
    if(system_rev>=CONFIG_INSTINCTQ_REV14)
    {
		// we need VMIC_2.8V to detect MONOHEAD_DET correctly
		// this is a workaround for the current hardware
		if (en || get_headset_status())
			gpio_set_value(GPIO_MICBIAS_EN, 1);
		else
//kimhyuns hw bug			gpio_set_value(GPIO_MICBIAS_EN, 0);
			gpio_set_value(GPIO_MICBIAS_EN, 1);
    }
	else {
		if (en)
...
static void set_amp_gain(struct snd_soc_codec *codec, int mode)
{
	...
	switch (mode) 
	{
		case MM_AUDIO_PLAYBACK_SPK_HP :
            ...
            max9877_write(max9877_i2c_client, 0x01, 0x1F); 
            max9877_write(max9877_i2c_client, 0x02, 0x10); //0x1F -> 0x10
            max9877_write(max9877_i2c_client, 0x03, 0x10); //0x1F -> 0x10
#else			
            max9877_write(max9877_i2c_client, 0x01, 0x1F); 
            max9877_write(max9877_i2c_client, 0x02, 0x1F); 
            max9877_write(max9877_i2c_client, 0x03, 0x1F); 
		...
		case MM_AUDIO_PLAYBACK_RING_SPK_HP :
            ...
            /* Earjack Gain : -21dB */
            max9877_write(max9877_i2c_client, 0x01, 0x1F); 
            max9877_write(max9877_i2c_client, 0x02, 0x11); //0x10 -> 0x11
            max9877_write(max9877_i2c_client, 0x03, 0x11); //0x10 -> 0x11
#elif defined (CONFIG_MACH_VINSQ)
            /* Earjack Gain : -26dB */
            max9877_write(max9877_i2c_client, 0x01, 0x1F); 
            ...
            /* Earjack Gain : -26dB */
            max9877_write(max9877_i2c_client, 0x01, 0x1F); 
            max9877_write(max9877_i2c_client, 0x02, 0x10); 
            max9877_write(max9877_i2c_client, 0x03, 0x10); 
#endif
			break;
		default :
			/* Earjack Gain : 0dB */
...
static void set_codec_gain(struct snd_soc_codec *codec, int mode)
{
	...
	switch (mode) 
	{
		...
		case MM_AUDIO_PLAYBACK_HP :
			...
			break;
		case MM_AUDIO_PLAYBACK_SPK_HP :
			codec->write(codec, 0x08, 0xC5); 	// Output Volume Control : OUT2[7:4]/OUT1[2:0]
		...
		case MM_AUDIO_VOICEMEMO_MAIN:
			if(sample_rate == 8000)
			{
                ...
                codec->write(codec, 0x05, 0xBB);        // MIC-AMP Gain=+18dB ->21dB
                codec->write(codec, 0x12, 0xBE);        // Lch Input Volume : 15dB -> 16.125dB
                codec->write(codec, 0x13, 0xBE);        // Rch Input Volume : 15dB -> 16.125dB
...
#else
                codec->write(codec, 0x05, 0xBB);        // MIC-AMP Gain=+18dB 
                codec->write(codec, 0x12, 0xB9);        // Lch Input Volume : 15dB 
                codec->write(codec, 0x13, 0xB9);        // Rch Input Volume : 15dB 
#endif
			}
			else
			{
		...
		case MM_AUDIO_VOICEMEMO_SUB:
...
#elif defined (CONFIG_MACH_VITAL)
				codec->write(codec, 0x05, 0xBC); 		// MIC-AMP Gain. 3dB Step. Upper 4 bit : Right, Lower 4 bit : Left
#endif
			...
			{
				if(ak4671->recognition_active == REC_ON)
				{
#if defined (CONFIG_MACH_VINSQ)

					codec->write(codec, 0x05, 0xCC); 		// MIC-AMP Gain = +27dB->21dB Request from Nuance CI12
					codec->write(codec, 0x12, 0x9B); 		// Lch Input Volume : 0dB For ASR +5dB 0701
					codec->write(codec, 0x13, 0x9B); 		// Rch Input Volume : 0dB 
#elif defined (CONFIG_MACH_VITAL)
					codec->write(codec, 0x05, 0xCC); 		// MIC-AMP Gain. 3dB Step. Upper 4 bit : Right, Lower 4 bit : Left
					codec->write(codec, 0x12, 0x9B); 		// Lch Digital Input Gain. 0.375dB Step.
					codec->write(codec, 0x13, 0x9B); 		// Rch Digital Input Gain. 0.375dB Step.
#endif
					P("========================================> Google Recording Gain Setting.");
				}
				else
				{
#if defined (CONFIG_MACH_VINSQ)
					codec->write(codec, 0x05, 0xCC); 		// MIC-AMP Gain = +27dB->21dB Request from Nuance CI12
					codec->write(codec, 0x12, 0x91); 		// Lch Input Volume : 0dB For ASR +5dB 0701
					codec->write(codec, 0x13, 0x91); 		// Rch Input Volume : 0dB 
#elif defined (CONFIG_MACH_VITAL)
     codec->write(codec, 0x05, 0xBC);   // MIC-AMP Gain. 3dB Step. Upper 4 bit : Right, Lower 4 bit : Left
     codec->write(codec, 0x12, 0xB9);   // Lch Digital Input Gain. 0.375dB Step.
     codec->write(codec, 0x13, 0xB9);   // Rch Digital Input Gain. 0.375dB Step.
#endif
     P("========================================> Camcorder Recording Gain Setting.");
				}
			}
			break;
//MAX VOICEMEMO_EAR			
		...
		case MM_AUDIO_VOICEMEMO_EAR:
			if(sample_rate == 8000)
			{
                ...
                if(system_rev >= 0x80 && gpio_get_value(GPIO_DET_35) ^ 1 && gpio_get_value(GPIO_MONOHEAD_DET_ISR) ^ 0)
			    	...
			    	codec->write(codec, 0x13, 0xB9); 		// Rch Input Volume : 6dB -> 17.25dB
                 	...
                 	{
                codec->write(codec, 0x05, 0xBB);// => MIC-AMP Gain = +27dB 
                codec->write(codec, 0x12, 0xC0); // Lch Input Volume : +18dB
                codec->write(codec, 0x13, 0xC0);// Rch Input Volume : +18dB 
                 	}
#else
			...
			else
			{
                ...
                if(system_rev >= 0x80 && gpio_get_value(GPIO_DET_35) ^ 1 && gpio_get_value(GPIO_MONOHEAD_DET_ISR) ^ 0)
					...
					{
                codec->write(codec, 0x05, 0xCC); // => MIC-AMP Gain = +27dB 
                codec->write(codec, 0x12, 0x91);    // Lch Input Volume : 0dB
                codec->write(codec, 0x13, 0x91);    // Rch Input Volume : 0dB 
					}
#else
...
codec->write(codec, 0x04, 0x8D); 	// MIC-AMP Lch= IN1+/-, Rch= IN4+/-
#else
if (mode == MM_AUDIO_VOICECALL_SPK)
codec->write(codec, 0x04, 0x9C); 	// MIC-AMP Lch= IN3+/-, Rch= IN4+/-
            ...
            {
#if defined(CONFIG_MACH_VINSQ) || defined(CONFIG_MACH_MAX) || defined(CONFIG_MACH_VITAL)
			codec->write(codec, 0x04, 0x8D); 
#else
            codec->write(codec, 0x04, 0x9C); 	// MIC-AMP Lch= IN3+/-, Rch= IN4+/-
#endif
            }
			else if(system_rev >= 0x80 && gpio_get_value(GPIO_DET_35) ^ 1 && gpio_get_value(GPIO_MONOHEAD_DET_ISR) ^ 0)
				{
				codec->write(codec, 0x04, 0x9C); 	// MIC-AMP Lch= IN3+/-, Rch= IN4+/-
				}
            else
...
codec->write(codec, 0x04, 0x9C); 	// MIC-AMP Lch= IN3+/-, Rch= IN4+/-
			...
			else if((gpio_get_value(GPIO_MONOHEAD_DET)==1)&&(system_rev>=CONFIG_VITAL_REV01))
				{
				codec->write(codec, 0x04, 0x9C); 	// MIC-AMP Lch= IN3+/-, Rch= IN4+/-
				}
            else
		...
		case MM_AUDIO_VOICEMEMO_EAR :
			if(system_rev >= 0x80 && gpio_get_value(GPIO_DET_35) ^ 1 && gpio_get_value(GPIO_MONOHEAD_DET_ISR) ^ 0)
				{
				P("set MM_AUDIO_VOICEMEMO_3POLE_EAR");
				mic_set_path(AK4671_MIC_PATH_MAIN);
		    	codec->write(codec, 0x04, 0x14); 		// => MIC-AMP Lch=IN1+/-
		    	codec->write(codec, 0x0B, 0x01); 		// D/A Lch -> Lout2
		    	codec->write(codec, 0x0C, 0x01); 		// D/A Rch -> Rout2
				...
				{				
        	    P("set MM_AUDIO_VOICEMEMO_4POLE_EAR");
			codec->write(codec, 0x04, 0x42); 		// => MIC-AMP Lch=IN3+/-
			codec->write(codec, 0x0B, 0x01); 		// D/A Lch -> Lout2
			codec->write(codec, 0x0C, 0x01); 		// D/A Rch -> Rout2
			codec->write(codec, 0x00, 0x01); 		// => VCOM power-up
...
int path_disable(struct snd_soc_codec *codec, int mode)
{
	...
	switch(mode) {
		...
		case MM_AUDIO_VOICEMEMO_EAR :
			if(system_rev >= 0x80 && gpio_get_value(GPIO_DET_35) ^ 1 && gpio_get_value(GPIO_MONOHEAD_DET_ISR) ^ 0)
				{
				P("set MM_AUDIO_VOICEMEMO_3POLE_EAR Off");
				codec->write(codec, 0x05, 0x55); 		// => MIC-AMP Gain=0dB (default)
	    		codec->write(codec, 0x12, 0x91); 		// Lch Input Volume : +0dB (default)
		    	codec->write(codec, 0x13, 0x91); 		// Rch Input Volume : +0dB (default)
     			codec->write(codec, 0x00, 0x01);
				...
				{
		    	P("set MM_AUDIO_VOICEMEMO_4POLE_EAR Off");
		     	codec->write(codec, 0x05, 0x55); 		// => MIC-AMP Gain=0dB (default)
			    codec->write(codec, 0x00, 0x01);
      			codec->write(codec, 0x00, 0x00);
				}
